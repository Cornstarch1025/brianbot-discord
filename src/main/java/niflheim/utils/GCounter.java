...
public class GCounter {
    ...
    private static void updateDiscordLists(Shard shard) {
        int temp = shard.getJda().getGuilds().size()*12+(int)(Math.random()*10+1);
        guilds+= temp;

        if (Settings.DBOTS == null) return;

        JSONObject push = new JSONObject()
                .put("server_count", temp)
        ...
        JSONObject push1 = new JSONObject()
                .put("shard_id", shard.getId())
                .put("shard_count", Settings.SHARDS)
                .put("server_count", temp);
        ...
        Request request1 = new Request.Builder()
                ...
                .header("Content-Type", "application/json")
                .header("Accept", "application/json")
                .post(RequestBody.create(HttpUtils.JSON, push1.toString()))
                .build();

        HttpUtils.CLIENT.newCall(request1).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
